...
class ListeDeCommunautProvider with ChangeNotifier {
  ...
  Future<void> fetchAllCommunities() async {
    ...
    try {
      ...
      _errorMessage =
          "Erreur lors de la récupération des communautés: ${e.toString()}";
      print("Error: $e");
    } finally {
      _isLoading = false;
      notifyListeners();
  ...
  Future<void> deleteCommunity(String projectId, String communityId) async {
    ...
    try {
      print(
          "Deleting community with ID: $communityId under project ID: $projectId for user: $userId");
      ...
      throw Exception(
          "Erreur lors de la suppression de la communauté: ${e.toString()}");
    }
  }
  ...
  Future<void> updateCommunity(
    ...
    try {
      ...
      _errorMessage =
          "Erreur lors de la mise à jour de la communauté: ${e.toString()}";
  ...
  Future<void> fetchAllJoinCommunities() async {
    ...
    try {
      ...
      for (var joinCommunityDoc in joinCommunitiesSnapshot.docs) {
        ...
        String description =
            joinCommunityData['description'] ?? 'No description available';
        ...
        // Fetch image from Firebase Storage if URL is empty
        if (imageUrl.isEmpty) {
          try {
            final storageRef = FirebaseStorage.instance
                .ref()
                .child('community_img/${joinCommunityDoc.id}.jpg');
            imageUrl = await storageRef.getDownloadURL();
          } catch (e) {
            print('Error fetching image for community $name: $e');
            imageUrl = 'default_image_url'; // Fallback image URL
          }
        }
